...
$(document).ready(function () {
    ...
    $("html,body").on("DOMMouseScroll mousewheel scroll touchmove", function (event) {
        var $currentSection = $(".slide.selected .content"),
            scrollsize = Math.ceil(Math.abs(event.deltaX) * event.deltaFactor),
            browserScrollRate = window.isFirefox ? 2 : 1,
            OSScrollRate = window.isWindows ? browserScrollRate * 2 : browserScrollRate,
            wheelDelta = event.originalEvent.wheelDelta ? event.originalEvent.wheelDelta : event.deltaX * event.deltaFactor,
            energy = wheelDelta * browserScrollRate * OSScrollRate,
            scrollDirection = event.deltaX >= 0 ? "left" : "right",
            ...
            if (scale > -1 && scale < 1 && !(window.allowParallaxOnMobile && window.isMobile)) {
                if ($element.hasClass("parallax") || $element.find(".parallax-element")) {
                    $element.find(".parallax-element").each(function () {
                        ...
                        $el.css("-webkit-transform", "translate3d(" + precentage + "%, 0, 0)").css("transform", "translate3d(" + precentage + "%, 0, 0)");
                    });
                }
            }
        });
    ...
    $(".mobile .slides:not(.scroll):not(.simplifiedMobile), .slides.desktopSwipe").swipe({
        swipeStatus: function (event, phase, direction, distance) {
            window.allowSwipeLeft = 1;
            window.allowSwipeRight = 1;
            var $currentSection = $(".slide.selected .content"),
...
//	var photoCount = 12; // the number of images in the slideshow
var fingerCount = 0;
var startX = 0;
var startY = 0;
var curX = 0;
...
function processingRoutine() {
    ...
    if (swipeDirection == "left") {
        ...
        //|| swipeDirection == 'down'
        event.preventDefault(); //left arrow
        if (window.horizontalMode) {
            event.preventDefault();
            window.changeSlide("decrease");
        }
    } else if (swipedElement == "photo-scroll" && swipeDirection == "down") {
        if (window.horizontalMode) {
        }
    }
}
